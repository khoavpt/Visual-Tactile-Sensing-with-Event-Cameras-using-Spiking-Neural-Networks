...
class SpikingCBAM(snn.SpikingNeuron):
    ...
    def __init__(self, 
        channels, kernel_size=7, reduction_ratio=2,
    ...
    def forward(self, x):
        ...
        self.channel_spikes = channel_attention.squeeze(3)[0] # (channels, 1)
        # Register spatial spikes at the first channel where channel attention is not 0
        non_zero_indices = torch.nonzero(channel_attention.squeeze(3)[0], as_tuple=True) # tuple of tensors of shape 

        self.spatial_spikes = spatial_attention[non_zero_indices[0]][0] # (height, width)
...
class SConv2dLSTM_CBAM(SpikingNeuron):
    ...
    def _base_state_function(self, input_):
        ...
        # Compute gates using spiking activations
        i = self.fire(cc_i)  # Input gate
        f = self.fire(cc_f)  # Forget gate pre-CBAM
        f = self.cbam(f)     # Apply SCBAM to forget gate
        o = self.fire(cc_o)  # Output gate
        g = self.fire(cc_g)  # Cell gate

        base_fn_syn = f * self.syn + i * g
        base_fn_mem = o * base_fn_syn
