...
class SparseConvSpikingBlock(nn.Module):
    ...
    def forward(self, x, mem_init):
        ...
        for t in range(sequence_length):
            ...
            indices = torch.nonzero(x_t)  # Get indices of non-zero elements
            features = x_t[tuple(indices.t())]  # Get corresponding values
            ...
            conv_out = self.conv(sparse_x)
            
            # Convert back to dense for batch norm
            dense_out = conv_out.dense()
            
            # Apply batch norm
            bn_out = self.bn(dense_out.unsqueeze(1))[:, 0]
            
            # Apply LIF and pooling
            spk, mem = self.lif(bn_out, mem)
            spk = self.pooling_layer(spk)
            spk_list.append(spk)
